...
Public Function Test7700CardsShouldBuildScanLists() As Assert
    ...
    If p_outcome.AssertSuccessful Then
        p_channelNumber = p_channelNumber + p_card.Capacity - 1
        Set p_card = p_routeSystem.SelectMultiplexerCard(p_channelNumber)
        Set p_outcome = Assert.IsNotNothing(p_card, _
            "A card should be selected for channel " & CStr(p_channelNumber) & ".")
    End If
    ...
    p_expectedFunctionScanList = ":FUNC 'RES',(@101:120)"
    Dim p_actualFunctionScanList As String
    Dim p_measurement As String: p_measurement = "RES"
    
    If p_outcome.AssertSuccessful Then
        p_actualFunctionScanList = p_card.BuildFunctionScanList(p_measurement)
    ...
    p_expectedFunctionScanList = ":FUNC 'RES',(@201:240)"
    p_measurement = "RES"
